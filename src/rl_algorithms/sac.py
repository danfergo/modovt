...
class DDPG:
    ...
    def update(self, policy_targ, q_fn_targ, replay_buffer):
        # def target_fn(timestep):
        #     o, a, r, o1, d = timestep
        #     return r + self.gamma * q_fn_targ(o1, policy_targ(o1))

        for i in range(self.n_update_batches):
            batch = replay_buffer.sample_batch(self.update_batch_size)

            # compute targets
            rs = torch.tensor(np.array([r for o, a, r, o1, d in batch]))
            aa = torch.cat([a for o, a, r, o1, d in batch], dim=0)
            os = torch.cat([o1 for o, a, r, o1, d in batch], dim=0)
            ...
            loss = torch.mean(self.q_fn(os, aa) - targets)
